...
class RoPE3D(BaseRoPE):
    def __init__(self, embed_dim, max_patch_resolution=(40, 160, 160), interpolation_scale=1, theta_time=10000.0, theta_space=10000.0, embed_dim_time=None, embed_dim_space=None, repeat_interleave=True, multi_id=False):
        ...
        self.freqs_cos = freqs_cos # 64, 256, 256, 512
        self.freqs_sin = freqs_sin # 64, 256, 256, 512
...
if __name__ == "__main__":
    ...
    关键断点位置建议：
    1. compute_position_embedding() 中的 position_t/h/w 计算
    2. apply_rope() 中的 x.reshape(..., -1, 2) 配对操作
    3. _forward() 中的 torch.narrow 裁剪操作
    """
    ...
    # 视频配置：41帧，原始分辨率64×128像素
    # 假设 patch_size = 8，则 patch 分辨率为 8×16
    T = 41
    H = 64 // 8   # = 8
    W = 128 // 8  # = 16
    num_patches = T * H * W  # = 5248
    
    # ============ 创建 RoPE3D ============
    rope = RoPE3D(
        embed_dim=embed_dim,
        max_patch_resolution=(64, 256, 256),  # 需要 >= 实际使用的分辨率
        theta_time=10000.0,
        theta_space=10000.0,
        multi_id=True,  # True时前2帧使用负位置编码
    )
    
    # ============ 创建输入张量 ============
    # x 模拟 attention 中的 Q 或 K
    ...
    print(f"freqs_cos 预计算形状: {rope.freqs_cos.shape}")
    
    # 验证形状不变
    assert x.shape == output.shape, "输入输出形状应相同"
    
    # 验证值发生了变化（位置编码生效）
    assert not torch.allclose(x, output), "输出应该与输入不同"
