...
def FlowMatchSFTLoss_mask(pipe: BasePipeline, **inputs):
    ...
    # sigma = pipe.scheduler.sigmas[timestep_id].to(device=pipe.device, dtype=pipe.torch_dtype)
    sigma = pipe.scheduler.sigmas[timestep_id].item()

    B, C, F, H, W = inputs["input_latents"].shape
    frame_sigma = torch.ones(B, 1, F, 1, 1, device=pipe.device, dtype=pipe.torch_dtype) * sigma
    frame_sigma[:, :, 0:1, :, :] = 0  # 首帧 sigma=0，不加噪

    # loss_mask: [B, 1, F, 1, 1]，首帧为0（不参与loss），其他帧为1
    loss_mask = torch.ones(B, 1, F, 1, 1, device=pipe.device, dtype=pipe.torch_dtype)
    loss_mask[:, :, 0:1, :, :] = 0  # 首帧不参与loss计算

    # inputs["latents"] = pipe.scheduler.add_noise(inputs["input_latents"], noise, timestep)
    inputs["latents"] = (1 - frame_sigma) * inputs["input_latents"] + frame_sigma * noise
    # training_target = pipe.scheduler.training_target(inputs["input_latents"], noise, timestep)
    ...
    loss = masked_diff.sum() / (loss_mask.sum() * C * H * W)
