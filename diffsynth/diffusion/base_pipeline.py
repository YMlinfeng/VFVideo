...
class PipelineUnitRunner:
    ...
    def __call__(self, unit: PipelineUnit, pipe: BasePipeline, inputs_shared: dict, inputs_posi: dict, inputs_nega: dict) -> tuple[dict, dict]: #type:ignore
        if unit.take_over: #s2v
            ...
            # 步骤 A: 照单抓药
            # unit.input_params 是这个工位的“需求清单”，比如 ['input_video', 'input_audio']
            # inputs_shared 是“大仓库”
            # 这行代码的意思是：只从仓库里取出清单上有的东西，打包成 processor_inputs
            processor_inputs = {name: inputs_shared.get(name) for name in unit.input_params}
            # 步骤 B: 只有原料，开始加工
            # **processor_inputs 是 Python 的解包语法，把字典变成关键字参数传进去
            # 比如 process(pipe, input_video=..., input_audio=...)
            processor_outputs = unit.process(pipe, **processor_inputs)
            # 加工好的东西（比如把图片变成了 Latent），放回大仓库，供下一个工位使用
