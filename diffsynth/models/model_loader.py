...
class ModelPool:
    ...
    def auto_load_model(self, path, vram_config=None, vram_limit=None, clear_parameters=False):
        ...
        # 强制使用s2v的model class
        # real_hash = hash_model_file(path)
        # target_s2v_hash = '966cffdcc52f9c46c391768b27637614' # Wan2.2-S2V-14B
        # is_dit_model = False
        # if isinstance(path, list):
        #     # 只要列表里有一个文件名包含关键词，就认为是 DiT
        #     for p in path:
        #         if "diffusion_pytorch_model" in str(p):
        #             is_dit_model = True
        #             break
        # # 2. 如果 path 是字符串 (单文件模型)
        # elif isinstance(path, str):
        #     if "diffusion_pytorch_model" in path:
        #         is_dit_model = True
        # if is_dit_model:
        #     print(f"正在伪造 DiT Hash，以便用 S2V 架构加载 I2V 权重: {path}")
        #     print(f"推理阶段可以忽略上条日志")
        #     model_hash = target_s2v_hash
        # else:
        #     print(f"正常加载其他模型: {path}")
