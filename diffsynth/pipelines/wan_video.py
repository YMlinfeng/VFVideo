...
class WanVideoPipeline(BasePipeline):
    ...
    @staticmethod # 静态方法不需要 self 或 cls 参数，可以直接通过类名调用
    def from_pretrained(
        torch_dtype: torch.dtype = torch.bfloat16,
        device: Union[str, torch.device] = "cuda",
        model_configs: list[ModelConfig] = [], #type:ignore
        ...
        '''
        从硬盘把训练好的 .pth 或 .safetensors 文件读进来，填入上面定义的 vae、dit 等组件里
        '''
        # Redirect model path
    ...
    def __call__(
        ...
        # Video-to-video
        input_video: Optional[list[Image.Image]] = None, #type:ignore
        denoising_strength: Optional[float] = 1.0,
        # Speech-to-video
        input_audio: Optional[np.array] = None,
        audio_embeds: Optional[torch.Tensor] = None,
        audio_sample_rate: Optional[int] = 16000,
        s2v_pose_video: Optional[list[Image.Image]] = None, #type:ignore
        s2v_pose_latents: Optional[torch.Tensor] = None, #type:ignore
        motion_video: Optional[list[Image.Image]] = None, #type:ignore
        # ControlNet
        control_video: Optional[list[Image.Image]] = None, #type:ignore
        ...
        # VACE
        vace_video: Optional[list[Image.Image]] = None, #type:ignore
        vace_video_mask: Optional[Image.Image] = None,
        vace_reference_image: Optional[Image.Image] = None,
        vace_scale: Optional[float] = 1.0,
        # Animate
        animate_pose_video: Optional[list[Image.Image]] = None, #type:ignore
        animate_face_video: Optional[list[Image.Image]] = None, #type:ignore
        animate_inpaint_video: Optional[list[Image.Image]] = None, #type:ignore
        animate_mask_video: Optional[list[Image.Image]] = None, #type:ignore
        # VAP
        vap_video: Optional[list[Image.Image]] = None, #type:ignore
        ...
        # LongCat-Video
        longcat_video: Optional[list[Image.Image]] = None, #type:ignore
        # VAE tiling
        tiled: Optional[bool] = True,
        tile_size: Optional[tuple[int, int]] = (30, 52), #type:ignore
        tile_stride: Optional[tuple[int, int]] = (15, 26), #type:ignore
        # Sliding window
        ...
        **kwargs,
        ...
        id_cfg_start_step = kwargs.get("id_cfg_start_step", 0)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if orig_inputs_nega_context is not None:
                if text_cfg_start_step <= progress_id <= text_cfg_end_step:
                    inputs_nega["context"] = orig_inputs_nega_context
                else:
                    inputs_nega["context"] = inputs_posi.get("context")
...
class WanVideoUnit_NoiseInitializertodo(PipelineUnit):
    def __init__(self):
        super().__init__(
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image", "id_grid"),
            output_params=("noise", "id_grid_noise")
        )

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image, id_grid=None):
        import math
        length_main = (num_frames - 1) // 4 + 1
        if vace_reference_image is not None:
            f_vace = len(vace_reference_image) if isinstance(vace_reference_image, list) else 1
            length_main += f_vace
            
        shape_main = (1, pipe.vae.model.z_dim, length_main, height // pipe.vae.upsampling_factor, width // pipe.vae.upsampling_factor)
        num_elements_main = math.prod(shape_main)
        ...
        if id_grid is not None:
            if id_grid.ndim == 4:
                id_grid = id_grid.unsqueeze(0)
            b, c, t, h, w = id_grid.shape
            # 对于 T=1，(1-1)//4+1 = 1。对于 T=41，(41-1)//4+1 = 11。
            length_grid = (t - 1) // 4 + 1 if t > 1 else 1
            shape_grid = (b, pipe.vae.model.z_dim, length_grid, h // pipe.vae.upsampling_factor, w // pipe.vae.upsampling_factor)
            num_elements_grid = math.prod(shape_grid)
            
        # 一次性生成一大块连续噪声，确保同 Seed 下的稳定性
        total_elements = num_elements_main + num_elements_grid
        generator = None if seed is None else torch.Generator(device=rand_device).manual_seed(seed)
        total_noise_1d = torch.randn((total_elements,), generator=generator, device=rand_device, dtype=torch.float32)
        total_noise_1d = total_noise_1d.to(device=pipe.device, dtype=pipe.torch_dtype)        
        noise = total_noise_1d[:num_elements_main].reshape(shape_main)
        if shape_grid is not None:
            id_grid_noise = total_noise_1d[num_elements_main:].reshape(shape_grid)
        else:
            id_grid_noise = None
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        if pipe.scheduler.training:
            ...
            latents = pipe.scheduler.add_noise(input_latents, noise, timestep=pipe.scheduler.timesteps[0]) #todo
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        if isinstance(input_image, torch.Tensor): 
            ...
            if input_image.size != (width, height): print(f"[InferBug] 输入图像被调整为 ({width}, {height}") 
            image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
        if input_image is None or not pipe.dit.fuse_vae_embedding_in_latents:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        # i = input_image; i = (i + 1.0) / 2.0; plt.imsave("output0.png", F.to_pil_image(i.float()))
        if isinstance(input_image, torch.Tensor):
            resized_image = F.resize(input_image, (height, width)) 
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideoPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
        ...
        # 调用 pipe.audio_encoder (Wav2Vec2) 提取特征
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Timestep
    if dit.seperated_timestep and fuse_vae_embedding_in_latents: #f and f
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                if use_gradient_checkpointing_offload:
                    ...
                    x = block(x, context, t_mod, freqs) #vace #! this path
...
def model_fn_wans2v(
    ...
    seq_len_x = seq_len_x_global = x.shape[1] # global used for unified sequence parallel 16800
