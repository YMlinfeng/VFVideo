...
def draw_lines(img,start,end,st2d,en2d):
    r,g,b = np.ceil((start-end)/(np.abs((start-end)).max())*128+127).astype(np.uint8)
    img = cv2.line(img, st2d[:2], en2d[:2], (int(r),int(g),int(b)),thickness=4)
    return img
def render_incam(incam_video_path,pt_path,video_path,dw_path,smplx):
    ...
    random_beta=torch.randn(pred["smpl_params_incam"]['betas'][0].shape)
    ...
    for i, img_raw in tqdm(enumerate(reader), total=get_video_lwh(video_path)[0], desc=f"Rendering Incam"):
        ...
        for ll in lls:
            img = draw_lines(img,pred_c_joints[i,ll[0]],pred_c_joints[i,ll[1]],joints[i,ll[0]],joints[i,ll[1]])
...
for i in open(label_name).readlines():
    smplx_path =i.strip()
    out_path = smplx_path.replace('hmr4d_results.pt','3d_pose_norm.mp4')
