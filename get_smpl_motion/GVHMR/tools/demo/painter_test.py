...
def render_incam(incam_video_path,pt_path,video_path,dw_path,smplx):
    ...
    random_beta=torch.randn(pred["smpl_params_incam"]['betas'][0].shape)*5
...
for i in open(label_name).readlines():
    nnn+=1
    smplx_path =i.strip()
    out_path = smplx_path.replace('hmr4d_results.pt','3d_pose_enh.mp4')#smplx_path.replace('hmr4d_results.pt','3d_pose_enh.mp4')
