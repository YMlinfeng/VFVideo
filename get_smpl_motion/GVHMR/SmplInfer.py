...
class SmplInfer:
    ...
    def infer(self, input_file, output_result_path):
        ...
        if len(output_result_path) > 0:
            ...
            video_name_without_ext = os.path.splitext(video_name_with_ext)[
                0
            ]  # 去掉扩展名，得到 'video'
...
if __name__ == "__main__":
    ...
    print("time start:", time.time() - time0)
    ...
    for i in range(5):
        time0 = time.time()
        # input_file = "/ytech_milm/xujialin/personal_project/smpl3d/grpc-python-raven-opt/ykit_service/docs/example_video/tennis_{0}.mp4".format(
        #     i
        # )
        input_file = "/vh_share/lishunkai/smpl/GVHMR/docs/example_video/tennis.mp4"

        # output_file = "/ytech_milm/xujialin/personal_project/smpl3d/grpc-python-raven-opt/ykit_service/outputs-opt/"
        output_file = "/vh_share/lishunkai/smpl/GVHMR/outputs/test"
        
        result = smpl_infer.infer(input_file, output_file)
        print("time infer{0}:".format(i), time.time() - time0)
