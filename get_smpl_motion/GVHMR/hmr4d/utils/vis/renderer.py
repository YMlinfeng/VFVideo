...
# def perspective_projection_ori(x3d, K, R=None, T=None):
#     if R != None:
...
#     x3d = torch.matmul(K, x2d.transpose(-1, -2)).transpose(-1, -2)
#     x2d = torch.matmul(K, x2d.transpose(-1, -2)).transpose(-1, -2)[..., :2]
#     return x2d, x3d

def perspective_projection(x3d, K, R=None, T=None):

    batchsize = x3d.shape[0]
    x3d = torch.matmul(x3d, K.repeat((batchsize, 1, 1)).permute((0, 2, 1)))
    x3d[..., :2] = torch.div(x3d[..., :2], x3d[..., 2].reshape(batchsize, -1, 1))
    return x3d
...
class Renderer:
    def __init__(self, width, height, focal_length=None, device="cuda", faces=None, K=None, bin_size=None):
        ...
        # self.create_renderer_ras()
        self.create_rasterizer()
    ...
    # def create_renderer_ras(self):
    #     self.renderer_ras = MeshRendererWithFragments(
    #         rasterizer=MeshRasterizer(
    #             raster_settings=RasterizationSettings(
    #                 image_size=self.image_sizes[0], blur_radius=1e-5, bin_size=self.bin_size
    #             ),
    ...
    def update_bbox(self, x3d, scale=2.0, mask=None):
        ...
        if x3d.size(-1) != 3:
            ...
            x2d, _ = perspective_projection(x3d.unsqueeze(0), self.K, self.R, self.T.reshape(1, 3, 1))
    ...
    def render_mesh(self, vertices, background=None, colors=[0.8, 0.8, 0.8], VI=50):
        ...
        # 修复depth
        # results_, depth_ = self.renderer_ras(mesh, materials=materials, cameras=self.cameras, lights=self.lights)
        depth_ = self.rasterizer(mesh, cameras=self.cameras)
        depth_ = depth_.zbuf.squeeze().cpu().numpy()

        depth_ = np.flip(depth_, axis=[0,1])
        ...
        #depth = overlay_image_onto_background(depth, mask, self.bboxes, background[...,0].copy())
        #self.reset_bbox()
        # return image, depth
