...
import cv2
import numpy as np
from ..wis3d_utils import get_colors_by_conf
...
def draw_bbx_xyxy_on_image_batch(bbx_xyxy_batch, image_batch, mask=None, conf=None):
    ...
    n1 = len(bbx_xyxy_batch)
    n2 = len(image_batch)
    image_batch_out = []
    for i in range(n1):
        if use_conf:
            image_batch_out.append(draw_bbx_xyxy_on_image(bbx_xyxy_batch[i], image_batch[min(i,n2-1)], conf[i]))
        else:
            if mask is None or mask[i]:
                image_batch_out.append(draw_bbx_xyxy_on_image(bbx_xyxy_batch[i], image_batch[min(i,n2-1)]))
            else:
                image_batch_out.append(image_batch[min(i,n2-1)])
    return image_batch_out
...
def draw_coco17_skeleton_batch(imgs, keypoints_batch, conf_thr=0):
    n1 = len(imgs)
    n2 = len(keypoints_batch)
    keypoints_batch = to_numpy(keypoints_batch)
    imgs_out = []
    for i in range(n2):
        imgs_out.append(draw_coco17_skeleton(imgs[min(i,n1-1)], keypoints_batch[i], conf_thr))
    return imgs_out
