...
import argparse
from ..visualization import rotation_conversions as geometry


class joints2smpl:
    def __init__(self, num_frames, device, SMPL_MODEL_DIR='', fix_foot=False):
        self.device = device
        print("device : ", device)
        ...
        if len(SMPL_MODEL_DIR) > 0:
            config.SMPL_MODEL_DIR = SMPL_MODEL_DIR
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        print("init param ", pred_pose.shape, pred_betas.shape, pred_cam_t.shape)
        ...
        return thetas.clone().detach(), {'pose': new_opt_pose.reshape(self.batch_size, 72).clone().detach(), 'joints': new_opt_joints[:24].flatten().clone().detach(), 'betas': new_opt_betas.clone().detach(), 'cam': new_opt_cam_t.clone().detach()}
