...
def to_angle_axis(x, eps=1e-10):
    ...
    return angle[..., None], x[..., 1:] / (length[..., None] + eps)
...
def to_euler(x, order='zyx'):
    ...
    if order == 'zyx':
        ...
        return np.concatenate([
            np.arctan2(-2*(x2*x3-x0*x1), 2 * (x0 * x0 + x3 * x3) - 1),
            np.arcsin(np.clip(2.0 * (x1 * x3 + x0 * x2), -1.0, 1.0)),
            np.arctan2(2.0 * (x0 * x3 - x1 * x2), 2 * (x0 * x0 + x1 * x1) - 1),
        ], axis=-1)
    else:
