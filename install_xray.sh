...
else print_red "Error: unrecognized cuda/rocm version"; exit 1
fi
...
function read_xray_latest() {
    if [ -z "$arg_install_xray_ver" ]; then  # 避免重复wget
        ...
        if [ "$?" != '0' ] || [ -z "$arg_install_xray_ver" ]; then
            print_red "Error: failed to read latest xray from ${remote_url}"; exit 1;
        fi
    fi
...
function download_xray_pkg() {
    ...
    if [ ! -f "${arg_install_xray_pkg}" ]; then  # 避免重复wget
        if [ -z "${XRAY_INSTALL_DEV}" ]; then
            ...
            print_red "Error: XRAY_INSTALL_DEV should be like: https://...xray_....rpm"; exit 2;
        fi
        _download_="$(hostname -i)_${_name_}"
        echo "downloading xray pkg ${_download_} from ${_remote_} into dir $(pwd)" >&2 # outputs to stderr
        wget -qO "${_download_}" "${_remote_}" || { print_red "Error: download xray from ${_remote_} failed"; exit 2; }
...
function read_kccl_latests() {
    if [ -z "$arg_install_kccl_ver" ]; then  # 避免重复wget
        ...
        if [ "$?" != '0' ] || [ -z "$arg_install_kccl_ver" ]; then
            print_red "Error: failed to read latest kccl from ${remote_url}"; exit 1;
        fi
    fi
...
if [ "$arg_install_nccl_dir" == "all" ]; then hostfile=/etc/mpi/mpi-hostfile
    for ((j=10; j>=0; j--)); do if check_ssh; then break; else sleep 2s; fi; if [ "$j" == 0 ]; then exit 3; fi; done
    ...
    if ! ifconfig "$TCP_NIC" &>/dev/null ; then print_red "ERROR: cannot find valid tcp nic"; exit 3; fi
    handle_output=' 2>&1 | sed "s/^/[$(hostname)] /g" || { echo -e "\e[31m\e[100m xray install failed on $(hostname) \e[0m" ; echo 1 > /etc/xray_install_failed_flag; }'
    ...
    if [ -d "$ceph_dir" ]; then
        ...
        env -i PATH="$PATH" HOME="$HOME" SUDO_USER="$SUDO_USER" OPAL_PREFIX="$OPAL_PREFIX" XRAY_INSTALL_DEV="$XRAY_INSTALL_DEV" XRAY_INSTALL_FORCE="$XRAY_INSTALL_FORCE" \
            ...
            -mca plm_rsh_num_concurrent 600 -mca routed_radix 600 -mca btl_openib_allow_ib false \
            -x PATH -x HOME -x SUDO_USER -x XRAY_INSTALL_DEV -x XRAY_INSTALL_FORCE \
            bash -c "set -e; set -o pipefail; bash ${ceph_dir}/xray/$0 '${ceph_dir}' '${xray_pkg}' '${xray_ver}' '${kccl_ver}' $handle_output"
    ...
    if [ "$mpirun_return" != "0" ]; then print_red "Error: mpirun xray install failed"; exit 4; fi
    env -i PATH="$PATH" OPAL_PREFIX="$OPAL_PREFIX" \
        timeout 30s $mpirun --allow-run-as-root -pernode -hostfile $hostfile -mca plm_rsh_args '-o LogLevel=ERROR' \
        -mca btl tcp,self -mca pml ob1 -mca btl_tcp_if_include $TCP_NIC -mca oob_tcp_if_include $TCP_NIC \
        -x PATH bash -c 'if [ -f /etc/xray_install_failed_flag ]; then rm -f /etc/xray_install_failed_flag; exit 1; fi'
    mpirun_return="$?"
    if [ "$mpirun_return" != "0" ]; then print_red "Error: xray install failed on some nodes"; exit 4; fi
...
function apt_install_func() {
    ...
    if [ "${apt_updated}" != 1 ]; then mkdir -p /tmp; chmod 1777 /tmp;
        apt update &> /dev/null && { print_green "apt update success"; apt_updated=1; } || { print_red "apt update failed, retrying"; }
        if [ "${apt_updated}" != 1 ]; then apt update &> /dev/null && print_green "apt update success" || { print_red "apt update failed"; exit 5; }; fi
    fi
    apt-get install -y "$@"  &> /dev/null  # might fail for the 1st time
    apt-get install -y "$@"  &> /dev/null && print_green "apt install $* success" || print_red "apt install $* failed"
}
function yum_install_func() {
    yum install -y --nogpgcheck --skip-broken "$@" &> /dev/null  # might fail for the 1st time
    yum install -y --nogpgcheck --skip-broken "$@" &> /dev/null && print_green "yum install $* success" || print_red "yum install $* failed"
}
if [ "${_os_}" == "ubuntu" ]; then echo "updating apt dependencies"
    dpkg --configure -a &> /dev/null || true
    apt_install_func apt-utils net-tools iproute2 lldpd bind9-utils bind9-host ethtool iputils-ping sudo bc coreutils
    apt_install_func ibutils libtool libtool-bin librdmacm-dev libibumad-dev libpci-dev pciutils libudev-dev ibverbs-utils pkg-config
elif [ "${_os_}" == "centos" ]; then echo "updating yum dependencies"
    yum_install_func autoconf automake libtool m4 net-tools iproute lldpd bind-utils ethtool iputils sudo bc coreutils
    yum_install_func ibutils libtool rdma-core-devel pciutils-devel libudev-devel libibverbs-utils pkgconfig
fi
lldpd; pip3 install requests 2>/dev/null
...
if [ "$up_to_date" == "0" ]; then
    ...
    if [ "${_os_}" == "ubuntu" ]; then
        ...
        && print_green "xray apt install success" || { print_red "ERROR: xray apt install failed"; error=6; }
    else
        yum localinstall -y "${xray_pkg}" >/dev/null \
        && print_green "xray yum install success" || { print_red "ERROR: xray yum install failed"; error=6; }
    fi
...
mkdir -p "$KCCL_PATH" && ls "$KCCL_PATH" >/dev/null && cd "$KCCL_PATH" || { print_red "ERROR: cannot enter $KCCL_PATH"; exit 7; }
...
for kccl in $kccls; do
    if [ "$skip" != "1" ]; then
        ...
        if ! timeout 5s ldd $kccl >/dev/null; then
            print_red "ERROR: download from ${remote_url}$kccl to ${KCCL_PATH}/$kccl failed"; exit 7
        fi
    fi
